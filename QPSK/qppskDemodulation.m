...
for n = 1: outputDataLength
    ...
    %--------------------------------------------LoopFilter----------------------------------------------%
        ...
        if mk(n) == 1
            symbolLoopFilterBuffer(1) = (1/256)*errDetecerOutputData(n);
...
%>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BPSK Decoding<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    for n = 1:cnt-1
        if (pskAngle(n) > 0) && (pskAngle(n) < 90)       
            basebandSignal(n) = 1;
        elseif (pskAngle(n) > 90) && (pskAngle(n) < 180)  
            basebandSignal(n) = 3;
        elseif (pskAngle(n) > 180) && (pskAngle(n) < 270) 
            basebandSignal(n) = 2;
        else
            basebandSignal(n) = 0;
        end
