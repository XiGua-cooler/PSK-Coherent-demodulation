...
function [ ModSignal ] = m8pskDemodulation( SymData, SymNum, M, CarrFre, Band, fs )
    ...
    symbolRate            = Band;
    samplingDuration      = SymNum / symbolRate;
    ...
    pskAngle                        = zeros(1, outputDataLength);  % Defined as the demodulation constellation angle.
...
for n = 1:outputDataLength
    ...
    %-----------------------NCO------------------------%
        ...
        ncoCosOutputData(n+1) = sin(loopFilterOutputData(n));
        ncoSinOutputData(n+1) = cos(loopFilterOutputData(n));
...
%>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BPSK Decoding<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
    ...
    for n = 1:cnt-1
        if (pskAngle(n) > 0) && (pskAngle(n) < 45)       
            basebandSignal(n) = 3;
        elseif (pskAngle(n) > 45) && (pskAngle(n) < 90)  
            basebandSignal(n) = 2;
        elseif (pskAngle(n) > 90) && (pskAngle(n) < 135) 
            basebandSignal(n) = 6;
        elseif (pskAngle(n) > 135) && (pskAngle(n) < 180) 
            basebandSignal(n) = 7;
        elseif (pskAngle(n) > 180) && (pskAngle(n) < 225) 
            basebandSignal(n) = 5;
        elseif (pskAngle(n) > 225) && (pskAngle(n) < 270) 
            basebandSignal(n) = 4;
        elseif (pskAngle(n) > 270) && (pskAngle(n) < 315) 
            basebandSignal(n) = 0;
        else
            basebandSignal(n) = 1;
        end
